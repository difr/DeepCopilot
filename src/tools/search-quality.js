// Search-result ranking for web_search.
//
// Verbatim copy of better-deepseek search-quality.js, adapted to CommonJS:
// 'export function X' -> 'function X' with module.exports at the bottom;
// also 'use strict' added (CJS is not strict by default).
// No other changes.
//
// Source:  https://github.com/EdgeTypE/better-deepseek
// File:    src/content/files/search-quality.js
// Version: commit 82428b9 (2026-08-23, "fix(search): timeouts, configurable
//          providers, Unicode ranking (#148)"; file unchanged in later clone
//          HEAD 6b2cb45). Vendored 2026-09-02.
// License: MIT — Copyright (c) 2026 Çağrı DÜRÜ (see repo LICENSE).
//
// What's here: query-signal extraction (quoted phrases, site:, negative
// terms, years, script-aware important tokens incl. Han bigrams) and
// relevance scoring used by the provider-rotation chain in web-search.js.
// buildEffectiveSearchQuery is exported for the upcoming research mode.
...
function extractSearchSignals(query) {
...
function buildEffectiveSearchQuery(query, options = {}) {
  const normalizedQuery = cleanText(query);
...
function rankSearchResults(query, results, options = {}) {
